...
const STATUS_STYLES: Record<SslStatus, { pill: string; label: string }> = {
  valid:   { pill: "bg-green-100 text-green-700",  label: "Valid" },
  expiring:{ pill: "bg-amber-100 text-amber-700",  label: "Expiring" },
  expired: { pill: "bg-red-100 text-red-700",      label: "Expired" },
  error:   { pill: "bg-red-100 text-red-700",      label: "Error" },
  ...
  if (loading) {
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <p className="text-sm text-gray-400">Loading certificate info…</p>
      </div>
    );
  }
      ...
      {check?.error ? (
        ...
        <>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">Days Remaining</dt>
            ...
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">TLS Version</dt>
              <dd className="mt-0.5 font-medium text-gray-900">{check?.tls_version ?? "—"}</dd>
            </div>
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">Valid From</dt>
              <dd className="mt-0.5 text-gray-700">{formatDate(check?.valid_from ?? null)}</dd>
            </div>
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">Valid To</dt>
              <dd className="mt-0.5 text-gray-700">{formatDate(check?.valid_to ?? null)}</dd>
            </div>
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">Subject</dt>
              <dd className="mt-0.5 font-medium text-gray-900 truncate">
                {check?.subject_cn ?? "—"}
              </dd>
              {check?.subject_o && (
                <dd className="text-xs text-gray-500 truncate">{check.subject_o}</dd>
              )}
            </div>
            <div>
              <dt className="text-gray-500 text-xs uppercase tracking-wide">Issuer</dt>
              <dd className="mt-0.5 font-medium text-gray-900 truncate">
                {check?.issuer_cn ?? "—"}
              </dd>
              {check?.issuer_o && (
                <dd className="text-xs text-gray-500 truncate">{check.issuer_o}</dd>
              )}
            </div>
            {check?.serial_number && (
              <div className="col-span-2">
                <dt className="text-gray-500 text-xs uppercase tracking-wide">Serial Number</dt>
                <dd className="mt-0.5 font-mono text-xs text-gray-600 break-all">
                  {check.serial_number}
                </dd>
              </div>
            )}
            {check?.fingerprint_sha256 && (
              <div className="col-span-2">
                <dt className="text-gray-500 text-xs uppercase tracking-wide flex items-center">
                  SHA-256 Fingerprint
                  <CopyButton text={check.fingerprint_sha256} />
                </dt>
                <dd className="mt-0.5 font-mono text-xs text-gray-600 break-all leading-tight">
                  {check.fingerprint_sha256}
                </dd>
              </div>
            )}
            {sans.length > 0 && (
              <div className="col-span-2">
                ...
                <dd className="mt-1 flex flex-wrap gap-1">
                  ...
                  {sans.length > 10 && (
                    <span className="text-xs text-gray-400">+{sans.length - 10} more</span>
                  )}
                </dd>
              </div>
            )}
